/* -- 9 -- flight dynamics (orbital guidance) ---------------------------
   This is p7_flight.js with one change, in the guidance block: the
   gravity turn is held under a pitch profile so the ascent actually
   reaches orbit instead of arcing over and flying back into the ground.
   Everything else - the integrator, the propulsion model, the events,
   the debris and the pad animation - is identical.
   --------------------------------------------------------------------
...
const MAX_PITCH  = 1.5708;   // 90 degrees: thrust along the horizon

/* Pitch profile: the steepest angle from vertical the vehicle is allowed
   to have reached by a given altitude (metres -> radians). A pure gravity
   turn has no such limit, and that is exactly why it runs away - see the
   guidance block below. The shape is the real Falcon 9 one: a slow kick
   through the first few kilometres, most of the turn done between 10 and
   45 km, and the last of it flown out by the upper stage. */
const PITCH_PROGRAM = [
  [     0, 0.00], [  1200, 0.09], [  4000, 0.26], [ 10000, 0.45],
  [ 22000, 0.66], [ 40000, 0.84], [ 65000, 0.96], [110000, 1.05],
  [180000, 1.13]
];

const TARGET_ALT = 205000;   // insertion altitude the upper stage flies to
...
function step(dt) {
  ...
  if (FL.t < PITCH_T) {
    ...
    /* First stage, in air. Thrust is held along the velocity vector so the
       stack flies at no angle of attack, but that gravity turn is not
       stable on its own: its commanded angle IS the flight path angle, so
       the instant vertical speed stops growing the command swings to the
       horizon, the vertical component of thrust collapses, and the
       vehicle can never climb out of it again - it arcs over and flies
       back into the ground. Real ascent guidance will not trade away more
       vertical speed than the altitude it has already earned can afford,
       so the turn is held under the profile above. The gravity turn still
       leads while it is sane, which is nearly the whole booster burn, and
       the ceiling only catches it afterwards. */
    const bias = 0.092 * Math.exp(-(FL.t - PITCH_T) / 48);
    const grav = Math.atan2(FL.vx, Math.max(FL.vy, 1)) + bias;
    const ceiling = tableLookup(PITCH_PROGRAM, FL.alt);
    const wanted = clamp(Math.min(grav, ceiling), 0, MAX_PITCH);
    FL.pitch = damp(FL.pitch, wanted, 2.4, dt);
    if (FL.t > PITCH_T + 0.1) fireEvent('pitch', 'Pitch and roll program', false);

  } else if (FL.stage === 2 && T > 0) {
    /* Upper stage, in vacuum. There is no air left to fly a zero angle of
       attack for, so steer directly instead of following the velocity.
       Hold just enough thrust vertical to carry what gravity still takes
       off the vehicle once the horizontal speed is bearing part of the
       load, and put everything else into building that horizontal speed.
       The faster it goes the more the centrifugal term cancels gravity,
       the smaller the vertical share needed, until the stage is thrusting
       flat along the horizon - which is what orbital insertion is. */
    const climb  = clamp((TARGET_ALT - FL.alt) * 0.005, 0, 400);
    const centri = (FL.vx * FL.vx) / (P.Re + FL.alt);
    const need   = (g - centri) + (climb - FL.vy) * 0.008;
    const avail  = Math.max(T / FL.mass, 0.01);
    const wanted = clamp(Math.acos(clamp(need / avail, -1, 1)), 0, MAX_PITCH);
    FL.pitch = damp(FL.pitch, wanted, 1.6, dt);
  }
